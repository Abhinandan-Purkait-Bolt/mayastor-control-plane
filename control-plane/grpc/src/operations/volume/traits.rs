...
impl TryFrom<volume::Volume> for Volume {
    ...
    fn try_from(volume: volume::Volume) -> Result<Self, Self::Error> {
        let grpc_volume_definition = match volume.definition {
        ...
        let grpc_volume_state = match volume.state {
            Some(state) => state,
            None => return Err(ReplyError::unwrap_err(ResourceKind::Volume)),
        };
...
impl TryFrom<volume::ReplicaTopology> for ReplicaTopology {
    type Error = ReplyError;
    fn try_from(replica_topology: volume::ReplicaTopology) -> Result<Self, Self::Error> {
        let node = replica_topology.node.map(|node| node.into());
        let pool = replica_topology.pool.map(|pool| pool.into());
        let status = match ReplicaStatus::try_from(replica_topology.status) {
            Ok(status) => status,
            Err(_) => return Err(ReplyError::unwrap_err(ResourceKind::Volume)),
        };
...
impl TryFrom<volume::Topology> for Topology {
    type Error = ReplyError;
    fn try_from(topology: volume::Topology) -> Result<Self, Self::Error> {
        let topo = Topology {
...
impl TryFrom<volume::NodeTopology> for NodeTopology {
    type Error = ReplyError;
    fn try_from(node_topology: volume::NodeTopology) -> Result<Self, Self::Error> {
        let node_topo = match node_topology.topology {
...
impl TryFrom<volume::PoolTopology> for PoolTopology {
    type Error = ReplyError;
    fn try_from(pool_topology: volume::PoolTopology) -> Result<Self, Self::Error> {
        let pool_topo = match pool_topology.topology {
...
impl From<volume::LabelledTopology> for LabelledTopology {
    fn from(labelled_topo: volume::LabelledTopology) -> Self {
        LabelledTopology {
            exclusion: match labelled_topo.exclusion {
                Some(labels) => labels.value,
                None => HashMap::new(),
            },
            inclusion: match labelled_topo.inclusion {
                Some(labels) => labels.value,
                None => HashMap::new(),
            },
        }
...
impl From<volume::ExplicitNodeTopology> for ExplicitNodeTopology {
    fn from(explicit_topo: volume::ExplicitNodeTopology) -> Self {
...
impl From<volume::VolumePolicy> for VolumePolicy {
    fn from(policy: volume::VolumePolicy) -> Self {
        VolumePolicy {
...
impl TryFrom<volume::VolumeTarget> for VolumeTarget {
    type Error = ReplyError;
    fn try_from(volume_target: volume::VolumeTarget) -> Result<Self, Self::Error> {
        let target = VolumeTarget::new(
            volume_target.node_id.into(),
            match volume_target.nexus_id {
                Some(id) => match NexusId::try_from(id) {
                    Ok(nexusid) => nexusid,
                    Err(_) => return Err(ReplyError::unwrap_err(ResourceKind::Volume)),
                },
